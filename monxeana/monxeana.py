...
# This function is used to add peak specific data to the 'fit_parameter_dictionary' generated by 'fit_range_mult_crystal_ball'.
def manually_add_peak_data_to_fpd(
    fit_parameter_dictionary,
    peak_data_dictionary
):
    # looping over the peak numbers, that are the same within both 'fit_parameter_dictionary' and 'peak_data_dictionary'
    for key in peak_data_dictionary:
        # looping over the 
        for keyy in peak_data_dictionary[key]:
            # adding the respective peak data entry to the 'fit_parameter_dictionary' dictionary
            fit_parameter_dictionary[key].update({keyy : peak_data_dictionary[key][keyy]})
    return
...
def automatically_add_peak_data_to_fpd(fit_parameter_dictionary):
    # looping over the peak numbers resembling the entries within 'fit_parameter_dictionary'
    for key in fit_parameter_dictionary:
        # energy resolution
        fit_parameter_dictionary[key].update({"resolution" : {}})
        res, res_err = get_resolution(single_cb_fit_param_dict=fit_parameter_dictionary[key]["fit_data"], single_cb_fit_param_error_dict=fit_parameter_dictionary[key]["fit_data_errors"])
        fit_parameter_dictionary[key]["resolution"].update({"resolution_in_percent" : res})
        fit_parameter_dictionary[key]["resolution"].update({"resolution_error" : res_err})
        return
